<script lang="ts">
	...
	function handleGlobalKeydown(event: KeyboardEvent) {
		...
		const isTextareaFocused = textareaElement === document.activeElement;

		if ((event.metaKey || event.shiftKey) && !isTextareaFocused) {
			// focus on textarea when (meta or shift) + Q is pressed
			if (event.key === "Q") {
				event.preventDefault();
				textareaElement?.focus();
			}

			// open model selector when shift + M is pressed
			if (event.key === "M") {
				event.preventDefault();
				// dispatch("openModelSelector");
			}
		}
	}
...
<div class="flex min-h-full flex-1 flex-col" onpaste={onPaste}>
	<textarea
		...
		tabindex="0"
		inputmode="text"
		class="scrollbar-custom max-h-[8lh] w-full resize-none overflow-y-auto overflow-x-hidden border-0 bg-transparent px-2.5 py-2.5 outline-none focus:ring-0 focus-visible:ring-0 max-sm:text-[16px] sm:px-3"
